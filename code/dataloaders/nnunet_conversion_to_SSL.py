...
if __name__ == '__main__':
    ...
    with open((parse_args.pickle), "rb") as input_pickle:
        ...
        # print(pk_dict)
        for fold in range(len(pk_dict)):
            with open(os.path.join(path, 'train_per'+str(parse_args.percent)+'_fold'+str(fold)+'.txt'), 'w') as f:
                for it in pk_dict[fold]['train']:
                    f.write(it+'\n')
            with open(os.path.join(path, 'unsup_per'+str(parse_args.percent)+'_fold'+str(fold)+'.txt'), 'w') as f:
                for it in pk_dict[fold]['unsupervised']:
                    f.write(it+'\n')
            with open(os.path.join(path, 'val_per'+str(parse_args.percent)+'_fold'+str(fold)+'.txt'), 'w') as f:
                for it in pk_dict[fold]['val']:
                    f.write(it+'\n')
